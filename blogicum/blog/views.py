...
def index(request):
    ...
    Отображаются последние опубликованные посты с пагинацией (не более 10 записей на страницу).
    """
...
def get_published_posts_queryset():
    ...
    Фильтрует посты по статусу публикации, дате публикации и статусу категории.
    """
    return (
        Post.objects
...
class ProfileListView(BlogMixin, ListView):
    ...
    Если залогиненный пользователь просматривает свой профиль, отображаются все его посты.
    Иначе — только опубликованные посты.
    В шаблон передаётся объект профиля под ключом "profile" и посты (контекстное имя 'post_list').
...
class PostDetailView(DetailView):
    ...
    что пост опубликован, его дата публикации не позже текущего времени и категория опубликована.
...
class PostCreateView(LoginRequiredMixin, CreateView):
    ...
    Использует форму PostForm (которая должна содержать поле для загрузки изображения).
    При успешной отправке записи перенаправляет пользователя на его страницу профиля.
    """
    model = Post
    form_class = PostForm
...
class PostUpdateView(LoginRequiredMixin, UpdateView):
    ...
    Доступна только автору поста. Если текущий пользователь не является автором,
...
class CommentCreateView(LoginRequiredMixin, CreateView):
    ...
    Доступна только авторизованным пользователям.
    При успешном добавлении комментария происходит перенаправление на детальную страницу поста.
    URL должен содержать параметр: post_pk.
    """
    model = Comment
...
class CommentUpdateView(LoginRequiredMixin, UpdateView):
    ...
    URL должен содержать параметр: pk (идентификатор комментария).
    """
    model = Comment
    form_class = CommentForm
    template_name = "blog/comment.html"
...
class CommentDeleteView(LoginRequiredMixin, DeleteView):
    ...
    Перед удалением отображается подтверждающая страница (используется общий шаблон).
...
def edit_profile(request):
    ...
    Предоставляет форму для изменения личных данных (имя, фамилия, email, логин).
    При корректном заполнении данные сохраняются, и пользователь перенаправляется на свою страницу профиля.
    """
